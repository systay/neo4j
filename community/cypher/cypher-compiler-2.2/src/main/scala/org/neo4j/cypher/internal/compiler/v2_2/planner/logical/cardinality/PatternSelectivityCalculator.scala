...
case class PatternSelectivityCalculator(stats: GraphStatistics) extends Pattern2Selectivity with SelectivityCombiner {
  ...
  def apply(pattern: PatternRelationship)(implicit semanticTable: SemanticTable, selections: Selections): Selectivity = {
    ...
    val lhsCardinality = calculateLabelCardinality(labelsOnLhs)
    val rhsCardinality = calculateLabelCardinality(labelsOnRhs)
    val maxRelCount = lhsCardinality * rhsCardinality

    val relCount = calculateRelCardinalities(types, labelsOnLhs, labelsOnRhs, pattern.dir).reduce(_ + _)

    relCount / maxRelCount
  }

  private def calculateRelCardinalities(types: Seq[TokenSpec[RelTypeId]],
                                        labelsOnLhs: Seq[TokenSpec[LabelId]],
                                        labelsOnRhs: Seq[TokenSpec[LabelId]],
                                        dir: Direction) = types flatMap { typ =>
    for {
      ...
      (lhsLabel, typ, rhsLabel) match {
        ...
        case _ if dir == Direction.OUTGOING =>
          stats.cardinalityByLabelsAndRelationshipType(lhsLabel.id, typ.id, rhsLabel.id)

        case _ if dir == Direction.INCOMING =>
          stats.cardinalityByLabelsAndRelationshipType(rhsLabel.id, typ.id, lhsLabel.id)
  ...
  private def calculateLabelCardinality(specs: Seq[TokenSpec[LabelId]]): Cardinality = (specs map {
    case SpecifiedButUnknown() => Cardinality(0)
    case spec: TokenSpec[LabelId] => stats.nodesWithLabelCardinality(spec.id)
  }).reduce(_ + _)
