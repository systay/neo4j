...
/*
Reverts forms introduced early in the planning process that are unnecessarily complex
 */
case object simplifyPredicates extends Rewriter {
  override def apply(input: AnyRef) = instance.apply(input)

  private val instance: Rewriter = bottomUp(Rewriter.lift {
    // <exp> IN [<singleValue>] => <exp> = <singleValue>
    case in@In(exp, ListLiteral(values@Seq(singleValue))) if values.size == 1 =>
      Equals(exp, singleValue)(in.position)
