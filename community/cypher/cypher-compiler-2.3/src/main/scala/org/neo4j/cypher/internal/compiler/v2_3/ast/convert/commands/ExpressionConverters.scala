...
object ExpressionConverters {
  ...
  def toCommandExpression(expression: ast.Expression): CommandExpression = expression match {
    ...
    case e: ast.True => commands.True()
    case e: ast.False => commands.Not(commands.True())
    case e: ast.Literal => commandexpressions.Literal(e.value)
    case e: ast.Identifier => identifier(e)
    case e: ast.Or => commands.Or(toCommandPredicate(e.lhs), toCommandPredicate(e.rhs))
    case e: ast.Xor => commands.Xor(toCommandPredicate(e.lhs), toCommandPredicate(e.rhs))
    case e: ast.And => commands.Ands(List(toCommandPredicate(e.lhs), toCommandPredicate(e.rhs)))
    case e: ast.Ands => commands.Ands(e.exprs.map(toCommandPredicate).toList)
    case e: ast.Ors => commands.Ors(e.exprs.map(toCommandPredicate).toList)
    case e: ast.Not => commands.Not(toCommandPredicate(e.rhs))
    case e: ast.Equals => commands.Equals(toCommandExpression(e.lhs), toCommandExpression(e.rhs))
    case e: ast.NotEquals => commands.Not(commands.Equals(toCommandExpression(e.lhs), toCommandExpression(e.rhs)))
    case e: ast.RegexMatch => regexMatch(e)
    case e: ast.In => in(e)
    case e: ast.Like => like(e)
    case e: ast.NotLike => commands.Not(toCommandPredicate(ast.Like(e.lhs, e.pattern, e.caseInsensitive)(e.position)))
    case e: ast.IsNull => commands.IsNull(toCommandExpression(e.lhs))
    case e: ast.IsNotNull => commands.Not(commands.IsNull(toCommandExpression(e.lhs)))
    ...
    case e: ast.FilterExpression => commandexpressions.FilterFunction(toCommandExpression(e.expression), e.identifier.name, e.innerPredicate.map(toCommandPredicate).getOrElse(commands.True()))
    ...
    case e: ast.AllIterablePredicate => commands.AllInCollection(toCommandExpression(e.expression), e.identifier.name, e.innerPredicate.map(toCommandPredicate).getOrElse(commands.True()))
    case e: ast.AnyIterablePredicate => commands.AnyInCollection(toCommandExpression(e.expression), e.identifier.name, e.innerPredicate.map(toCommandPredicate).getOrElse(commands.True()))
    case e: ast.NoneIterablePredicate => commands.NoneInCollection(toCommandExpression(e.expression), e.identifier.name, e.innerPredicate.map(toCommandPredicate).getOrElse(commands.True()))
    case e: ast.SingleIterablePredicate => commands.SingleInCollection(toCommandExpression(e.expression), e.identifier.name, e.innerPredicate.map(toCommandPredicate).getOrElse(commands.True()))
    ...
    case e: ast.PathExpression => pathExpression(e)
    case e: ast.NestedPipeExpression => commandexpressions.NestedPipeExpression(e.pipe, pathExpression(e.path))
    ...
    case e: ast.AndedPropertyInequalities => commands.AndedPropertyComparablePredicates(identifier(e.identifier), toCommandProperty(e.property), e.inequalities.map(inequalityExpression))
    case _ =>
      throw new InternalException(s"Unknown expression type during transformation (${expression.getClass})")
  }

  def toCommandPredicate(e: ast.Expression): CommandPredicate = e match {
    case e: ast.PatternExpression => commands.NonEmpty(toCommandExpression(e))
    case e: ast.FilterExpression => commands.NonEmpty(toCommandExpression(e))
    case e: ast.ExtractExpression => commands.NonEmpty(toCommandExpression(e))
    case e: ast.ListComprehension => commands.NonEmpty(toCommandExpression(e))
  ...
  private def inequalityExpression(original: ast.InequalityExpression): commands.ComparablePredicate = original match {
    case e: ast.LessThan => commands.LessThan(toCommandExpression(e.lhs), toCommandExpression(e.rhs))
    case e: ast.LessThanOrEqual => commands.LessThanOrEqual(toCommandExpression(e.lhs), toCommandExpression(e.rhs))
    case e: ast.GreaterThan => commands.GreaterThan(toCommandExpression(e.lhs), toCommandExpression(e.rhs))
    case e: ast.GreaterThanOrEqual => commands.GreaterThanOrEqual(toCommandExpression(e.lhs), toCommandExpression(e.rhs))
  }
  ...
  private def regexMatch(e: ast.RegexMatch) = toCommandExpression(e.rhs) match {
    case literal: commandexpressions.Literal =>
      commands.LiteralRegularExpression(toCommandExpression(e.lhs), literal)
    case command =>
      commands.RegularExpression(toCommandExpression(e.lhs), command)
  }
  ...
  private def like(e: ast.Like) = {
    ...
    toCommandExpression(e.rhs) match {
      ...
      case commandexpressions.Literal(v) =>
        ...
        val regularExpression = commands.LiteralRegularExpression(toCommandExpression(e.lhs), literal)
        commands.LiteralLikePattern(regularExpression, pattern, e.caseInsensitive)

      case command =>
        commands.RegularExpression(toCommandExpression(e.lhs), command)(stringToRegex)
    }
  }

  private def in(e: ast.In) = {
    val innerEquals = commands.Equals(toCommandExpression(e.lhs), commandexpressions.Identifier("-_-INNER-_-"))
  ...
  private def hasLabels(e: ast.HasLabels) = e.labels.map {
    l => commands.HasLabel(toCommandExpression(e.expression), commandvalues.KeyToken.Unresolved(l.name, commandvalues.TokenType.Label)): CommandPredicate
  } reduceLeft {
    commands.And(_, _)
  }
  ...
  private def pathExpression(e: ast.PathExpression): ProjectedPath = {
