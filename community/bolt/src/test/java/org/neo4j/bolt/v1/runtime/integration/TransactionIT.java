...
public class TransactionIT
{
    ...
    public void shouldTerminateQueriesEvenIfUsingPeriodicCommit() throws Exception
    {
        ...
        Thread thread = new Thread()
        {
            ...
            public void run()
            {
                try ( BoltStateMachine stateMachine = env.newMachine( "<write>" ) )
                {
                    ...
                    try
                    {
                        System.out.println(Thread.currentThread().getId() + " starting latch");
                        latch.start();
                        stateMachine.run( query, emptyMap(), nullResponseHandler() );
                        System.out.println(Thread.currentThread().getId() + " query starting!");
                        stateMachine.pullAll( nullResponseHandler() );
                        System.out.println(Thread.currentThread().getId() + " query finished!");
                    }
                    finally
                    {
                        latch.finish();
        ...
        System.out.println(Thread.currentThread().getId() + " starting query thread");
        ...
        System.out.println(Thread.currentThread().getId() + " connection reset");

        barrier.release();
        System.out.println(Thread.currentThread().getId() + " server released!");
            ...
            int firstBatchSize, int otherBatchSize )
    {
        ...
        server.setHandler( new AbstractHandler()
        {
            ...
            ) throws IOException, ServletException
            {
                ...
                out.flush();
                latch.start();
                System.out.println(Thread.currentThread().getId() + " server blocking!");
                innerBarrier.reached();
                System.out.println(Thread.currentThread().getId() + " server running!");
