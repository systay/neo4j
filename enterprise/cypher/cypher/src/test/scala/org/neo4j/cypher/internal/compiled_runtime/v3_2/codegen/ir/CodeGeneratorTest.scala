...
abstract class CodeGeneratorTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("hash join on top of hash join") {
    ...
    val projection = PlanProjection(join2, Map("a" -> varFor("a")))(solved)
  ...
  test("project literal") {
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> SignedDecimalIntegerLiteral("1")(pos)))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("project parameter") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> Parameter("FOO", CTAny)(pos)))(solved))
  ...
  test("project nodes") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(scan, Map("a" -> varFor("a")))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("project relationships") { // MATCH (a)-[r]->(b) WITH r RETURN r
    ...
    val expand = Expand(
        AllNodesScan(IdName("a"), Set.empty)(solved), IdName("a"),
        SemanticDirection.OUTGOING, Seq.empty, IdName("b"), IdName("r"), ExpandAll)(solved)
    val projection = PlanProjection(expand, Map("r" -> varFor("r")))(solved)
  ...
  test("project addition of two ints") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> add))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("project subtraction of two ints") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> subtract))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("project collection") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> collection))(solved))
    val compiled = compileAndExecute(plan, Map("FOO" -> Long.box(3L), "BAR" -> Long.box(1L)))
  ...
  test("project map") {
    ...
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> map))(solved))
    val compiled = compileAndExecute(plan, Map("FOO" -> Long.box(3L), "BAR" -> Long.box(1L)))
  ...
  test("string equality") {
    ...
    val plan = ProduceResult(List("result"), PlanProjection(SingleRow()(solved), Map("result" -> equals))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("or between two literal booleans") {
    ...
    val plan = ProduceResult(List("result"), PlanProjection(SingleRow()(solved), Map("result" -> or))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("not on a literal") {
    ...
    val plan = ProduceResult(List("result"), PlanProjection(SingleRow()(solved), Map("result" -> not))(solved))
    val compiled = compileAndExecute(plan)
  ...
  test("close transaction after successfully exhausting result") {
    // given
    val plan = ProduceResult(List("a"), PlanProjection(SingleRow()(solved), Map("a" -> SignedDecimalIntegerLiteral("1")(null)))(solved))
  ...
  test("handle line breaks and double quotes in names") {
    ...
    val name = """{"a":
              ...
              |}
            """.stripMargin
    val plan = ProduceResult(List(name), PlanProjection(SingleRow()(solved),
  ...
  test("count property grouping key") {
    ...
    val invocation: FunctionInvocation = FunctionInvocation(ns, count, distinct = false, Vector(property))(pos)
    val projection = PlanProjection(scan, Map("a.prop" -> property))(solved)
  ...
  test("count nodes distinct property grouping key") {
    ...
    val node = ast.Variable("a")(pos)
    val invocation: FunctionInvocation = FunctionInvocation(ns, count, distinct = true, Vector(node))(pos)
    val projection = PlanProjection(scan, Map("a.prop" -> property, "a" -> node))(solved)
  ...
  test("unwind list of integers with projection") { // UNWIND [1, 2, 3] as x WITH x as y RETURN y
    ...
    val unwind = UnwindCollection(SingleRow()(solved), IdName("x"), listLiteral)(solved)
    val projection = PlanProjection(unwind, Map("y" -> varFor("x")))(solved)
  ...
  test("projection of int list") { // WITH [1, 2, 3] as x RETURN x
    ...
    val projection = PlanProjection(SingleRow()(solved), Map("x" -> listLiteral))(solved)
  ...
  test("compare nullable relationships with equals") {
    ...
    val projection = PlanProjection(optionalExpandInto, Map("sameRel" -> Equals(varFor("r1"), varFor("r2"))(pos)))(solved)
