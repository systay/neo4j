...
case class HasLabels(offset: Int, label: KeyToken) extends Predicate {
  ...
  override def isMatch(m: ExecutionContext)(implicit state: QueryState): Option[Boolean] = {
    val nodeId = m.getLong(offset)
    if (nodeId == -1)
      None
    else {
      val maybeToken = label.getOptId(state.query)
      maybeToken match {
        case Some(labelId) =>
          Some(state.query.isLabelSetOnNode(labelId, nodeId))
        case None =>
          Some(false)
      }
    }
