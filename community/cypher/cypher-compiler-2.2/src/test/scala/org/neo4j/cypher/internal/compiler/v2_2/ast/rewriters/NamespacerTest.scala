...
class NamespacerTest extends CypherFunSuite {
  ...
  //  """match me-[`  r1@10`:ATE]->food<-[r2:ATE]-you where id(me) = 1
  //    |with me,count(distinct r1) as H1,count(distinct r2) as H2,you match me-[r1:ATE]->food<-[r2:ATE]-you
  //    |return me,you,sum((1-ABS(r1.times/H1-r2.times/H2))*(r1.times+r2.times)/(H1+H2))""".stripMargin
  ...
  """MATCH (person:Person)<--(message)<-[like]-(liker:Person)
    |WITH message AS message, like.creationDate AS likeTime, person AS person
    |WITH likeTime AS likeTime, person AS person, message.id AS `  FRESHID138`
    |ORDER BY likeTime, `  FRESHID138`
    |WITH likeTime AS likeTime, person AS person
    |WITH collect({likeTime: likeTime}) AS `  AGGREGATION152`, person AS person
    |WITH head(`  AGGREGATION152`) AS latestLike, person AS person
    |WITH latestLike.likeTime AS `  FRESHID233`
    |ORDER BY `  FRESHID233`
    |RETURN `  FRESHID233` AS likeTime""".stripMargin
  )
