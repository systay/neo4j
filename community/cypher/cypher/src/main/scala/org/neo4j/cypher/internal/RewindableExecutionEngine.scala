...
class RewindableExecutionEngine(graph: GraphDatabaseService, logger: StringLogger = StringLogger.DEV_NULL)
  ...
  override def profile(query: String, params: Map[String, Any]): ExtendedExecutionResult =
    RewindableExecutionResult(super.profile(query, params))

  @throws(classOf[SyntaxException])
  override def execute(query: String, params: Map[String, Any]): ExtendedExecutionResult =
    RewindableExecutionResult(super.execute(query, params))
}
