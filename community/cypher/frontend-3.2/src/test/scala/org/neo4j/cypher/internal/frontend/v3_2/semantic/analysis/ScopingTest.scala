...
class ScopingTest extends CypherFunSuite {
  ...
  test("MATCH (a) RETURN *") {
    ...
    returnClause.myScope.value._1 should equal(Scope.empty.enterScope(ScopingContext.Default).add(Variable("a")(pos)))
  }
  ...
  test("MATCH (a) FOREACH(x in [1,2] | CREATE ())") {
    ...
    query.myScope.value._1 should equal(Scope.empty)
    singleQuery.myScope.value._1 should equal(Scope.empty)
    val matchScope = Scope.empty.enterScope(ScopingContext.Default).add(Variable("a")(pos))
    foreach.myScope.value._1 should equal(matchScope)
    val createScope = matchScope.enterScope(ScopingContext.Default).add(Variable("x")(pos))
  ...
  test("RETURN x should fail with missing variable") {
    ...
    intercept[InvalidSemanticsException](Scoping.enrich(query))
  }
