...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("match n return n.prop order by n.prop2 DESC") {
    ...
    val expectation =
      """PlannerQuery(QueryGraph(Set(),Set(IdName(n)),Set(),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(n -> Identifier(n),   FRESHID17 -> Property(Identifier(n),PropertyKeyName(prop))),QueryShuffle(List(),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(n), IdName(  FRESHID17)),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(  FRESHID17 -> Identifier(  FRESHID17),   FRESHID33 -> Property(Identifier(n),PropertyKeyName(prop2))),QueryShuffle(List(),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(  FRESHID17), IdName(  FRESHID33)),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(  FRESHID17 -> Identifier(  FRESHID17),   FRESHID33 -> Identifier(  FRESHID33)),QueryShuffle(Vector(DescSortItem(Identifier(  FRESHID33))),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(  FRESHID17), IdName(  FRESHID33)),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(n.prop -> Identifier(  FRESHID17)),QueryShuffle(List(),None,None),false),None)))))))""".stripMargin
  ...
  test("MATCH (a) WITH a WHERE TRUE RETURN a") {
    ...
    val expectation =
      """PlannerQuery(QueryGraph(Set(),Set(IdName(a)),Set(),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(a -> Identifier(a),   FRESHID23 -> True()),QueryShuffle(List(),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(a), IdName(  FRESHID23)),Selections(Set(Predicate(Set(IdName(  FRESHID23)),Identifier(  FRESHID23)))),List(),Set(),Set()),RegularQueryProjection(Map(a -> Identifier(a)),QueryShuffle(List(),None,None),false),None)))""".stripMargin
    result should equal(expectation)
  }
  ...
  test("WITH DISTINCT 1 as b RETURN b") {
    ...
    query.horizon should equal(RegularQueryProjection(
      projections = Map("b" -> SignedDecimalIntegerLiteral("1") _),
      distinct = true
    ))
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    ...
    val result = query.toString
    val expectation =
      """PlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),List(),Set(),Set()),AggregatingQueryProjection(Map(owner -> Identifier(owner)),Map(xyz -> CountStar()),QueryShuffle(List(),None,None)),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(xyz)),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(owner -> Identifier(owner), collection -> GreaterThan(Identifier(xyz),SignedDecimalIntegerLiteral(0))),QueryShuffle(List(),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection)),Selections(Set()),List(),Set(),Set()),RegularQueryProjection(Map(owner -> Identifier(owner), collection -> Identifier(collection),   FRESHID82 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Identifier(owner)),List(),None,false),RelationshipPattern(Some(Identifier(  UNNAMED90)),false,List(),None,None,BOTH),NodePattern(Some(Identifier(  UNNAMED92)),List(),None,false))))),QueryShuffle(List(),None,None),false),Some(PlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection), IdName(  FRESHID82)),Selections(Set(Predicate(Set(IdName(  FRESHID82)),Identifier(  FRESHID82)))),List(),Set(),Set()),RegularQueryProjection(Map(owner -> Identifier(owner)),QueryShuffle(List(),None,None),false),None)))))))""".stripMargin
