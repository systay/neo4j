...
/*
Used when we only need a Set of nodes. Uses less runtime by not enumerating paths that have already been seen
 */
case class DistinctVarExpand(left: LogicalPlan,
                             from: IdName,
                             dir: SemanticDirection,
                             types: Seq[RelTypeName],
                             to: IdName,
