...
import org.neo4j.kernel.impl.query.{Neo4jTransactionalContext, QuerySource, TransactionalContext}
...
class CompilerComparisonTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with NewPlannerTestSupport {
  ...
  private def runQueryWith(query: String, compiler: CypherCompiler, db: GraphDatabaseQueryService): (List[Map[String, Any]], InternalExecutionResult) = {
    ...
    val (executionPlan: ExecutionPlan, extractedParams: Map[String, Any]) = db.withTx { tx =>
      val transactionalContext = Neo4jTransactionalContext.create(graph, querySource, tx, db.statement, query, Collections.emptyMap(), locker)
