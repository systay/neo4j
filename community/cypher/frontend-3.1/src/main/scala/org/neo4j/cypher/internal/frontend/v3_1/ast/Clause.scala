...
case class LoadCSV(withHeaders: Boolean, urlString: Expression, variable: Variable, fieldTerminator: Option[StringLiteral]) extends Clause with SemanticChecking {
...
case class Foreach(variable: Variable, expression: Expression, updates: Seq[Clause]) extends UpdateClause with SemanticChecking {
...
case class Unwind(expression: Expression, variable: Variable) extends Clause {
...
sealed trait ProjectionClause extends HorizonClause with SemanticChecking {
  ...
  private def createSpecialReturnItems(previousScope: Scope, s: SemanticState): ReturnItems = {
    ...
    val specialReturnItems = returnItems.copy(includeExisting = includePreviousScope)
    specialReturnItems.position.update(this.position())
    specialReturnItems
  }
...
case class PragmaWithout(excluded: Seq[Variable]) extends HorizonClause {
