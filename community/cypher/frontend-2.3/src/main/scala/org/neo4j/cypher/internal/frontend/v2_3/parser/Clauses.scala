...
trait Clauses extends Parser
  ...
  def Pragma: Rule1[ast.Clause] = rule("") {
    keyword("_PRAGMA") ~~ (
        group(keyword("WITH NONE") ~ put(ast.ReturnItems(includeExisting = false, Seq())) ~~ optional(Skip) ~~ optional(Limit) ~~ optional(Where)) ~~>> (ast.With(distinct = false, _, None, _, _, _))
