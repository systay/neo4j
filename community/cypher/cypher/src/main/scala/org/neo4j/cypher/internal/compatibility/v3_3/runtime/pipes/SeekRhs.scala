...
object Seek {
  def unapply(v: Any): Option[(Expression, SeekRhs)] = v match {
...
case class SingleSeekRhs(expr: Expression) extends SeekRhs {
  ...
  override def asQueryExpression: SingleQueryExpression[Expression] =
    SingleQueryExpression(expr)

  override def asCommandSeekArgs: SeekArgs =
...
case class MultiSeekRhs(expr: Expression) extends SeekRhs {
  override val sizeHint: Option[Int] = expr match {
    case coll: ListLiteral => Some(coll.expressions.size)
    case _                => None
  }

  override def map(f: Expression => Expression): MultiSeekRhs = expr match {
    case coll: ListLiteral => copy(expr = coll.map(f))
    case _ => copy(expr = f(expr))
  }

  override def asQueryExpression: ManyQueryExpression[Expression] =
