...
class SimplifyPredicatesTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("should rewrite AndedPropertyInequalities when they contain single element") {
    val singleRow: LogicalPlan = Argument(Set(IdName("x")))(solved)(Map.empty)
    val propertyE = prop("x", "key")
    val lessThan = LessThan(propertyE, literalInt(42))(pos)
    val predicate: NonEmptyList[InequalityExpression] = NonEmptyList(lessThan)
    val orgPredicate: Expression = AndedPropertyInequalities(varFor("x"), propertyE, predicate)
    val selection = Selection(Seq(orgPredicate), singleRow)(solved)

    selection.endoRewrite(simplifyPredicates) should equal(Selection(Seq(lessThan), singleRow)(solved))
  }
}
