...
class ContainsFunctionTest extends CypherFunSuite {

  test("empty list") {
    val buildUp = new BuildUp(Iterator.empty)
    buildUp.contains(42) should equal((Some(false), AlwaysNotIn))
  }
  ...
  test("null in lhs on an empty list") {
    new BuildUp(Iterator.empty).contains(null) should equal(Some(false), AlwaysNotIn)
  }
  ...
  test("buildUp can handle maps on the rhs") {
    ...
    result should equal(Some(false))
    newChecker shouldBe a [SlowChecker]
  }
