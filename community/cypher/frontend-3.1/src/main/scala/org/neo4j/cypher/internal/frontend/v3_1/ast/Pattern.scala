...
case class NamedPatternPart(variable: Variable, patternPart: AnonymousPatternPart) extends PatternPart {
...
case class EveryPath(element: PatternElement) extends AnonymousPatternPart {
  override val position = element.position
...
object InvalidNodePattern {
  def apply(id: Variable, labels: Seq[LabelName], properties: Option[Expression]) =
    new InvalidNodePattern(id)
}

class InvalidNodePattern(val id: Variable) extends NodePattern(Some(id), Seq.empty, None) {
