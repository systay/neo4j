...
case class SetPropertyItem(property: Property, expression: Expression) extends SetItem {
  def semanticCheck =
...
case class SetLabelItem(expression: Expression, labels: Seq[LabelName]) extends SetItem {
  def semanticCheck =
    expression.semanticCheck(Expression.SemanticContext.Simple) chain
